...
const HeroCar = ({ car }: HeroCarProps) => {
  ...
  return (
    <section className="relative w-full aspect-[3/2] sm:h-[75vh] sm:aspect-auto overflow-hidden">
      <Image
        src={mainImage}
        alt={`${car.brand} ${car.model}`}
        fill
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-white/20 via-white/0 to-white/0" />

      {/* Название и цена — в левом верхнем углу */}
      <div className="absolute top-4 left-4 z-10 text-white">
        <h1 className="text-xl sm:text-4xl font-bold leading-tight">
          {`${car.brand} ${car.model}`}
        </h1>
        <p className="text-sm sm:text-xl mt-1">
          от ${formatPrice(car.price.usd)} ({formatPrice(car.price.tg)} ₸)
        </p>
      </div>

      {/* Индикатор скролла по центру снизу */}
      <motion.div
