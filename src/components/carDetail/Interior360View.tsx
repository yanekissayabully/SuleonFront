  ...
  return (
    <div className="my-8">
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        {/* Кнопка с подпрыгивающей иконкой */}
        <button
          onClick={() => setIsOpen(true)}
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-4 rounded-full shadow-lg transition-transform ${
            isBouncing ? 'transform -translate-y-1/3' : ''
          }`}
        >
          <Image
            src="/icons/icons-360.png"
      ...
      {isOpen && (
        ...
        >
          <div
            className="relative bg-white rounded-xl overflow-hidden w-full max-w-6xl mx-4 h-[80vh]"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setIsOpen(false)}
